...
### 화면 구조 (하단 탭 내비)
모바일 앱처럼 목적별 공간을 하단 탭 4개로 분리 (자세히: [docs/09](./docs/09_ux-restructure-plan.md)):
- **발견**: 큐레이션 홈 피드 — "오늘의 발견" 히어로 + 테마 가로 레일(내 근처·지금 영업중·국밥명가·한잔·혼밥·리뷰많은집). 전부 데이터 규칙으로 생성. 검색·필터를 걸면 기존 그리드로 폴백.
- **지도**: 다크 지도 + 접이식 하단 시트(목록·지도 동시). 목록 행을 누르면 해당 위치로 flyTo + 상세.
- **AI 추천**: 대화형 추천을 일급 진입점으로.
- **내 리스트**: 즐겨찾기·가본 곳·내 평가 세그먼트 + 공유 통합.
...
### 지도 뷰
- **Leaflet 인터랙티브 지도**: CARTO 다크 타일, 마커 클릭 시 팝업, 화면 꽉 채우는 높이
- **하단 시트**: 접이식 목록 오버레이 — 지도와 목록을 동시에 보고, 행 탭 시 해당 위치로 flyTo + 상세 모달
...
### 개인화
- **내 리스트 탭**: 즐겨찾기 · 가본 곳 · 내 평가(별점·메모 남긴 곳) 세그먼트로 통합, 각 카운트 표시
- **즐겨찾기** (♡): localStorage 저장
- **즐겨찾기 공유**: 내 리스트를 URL로 친구에게 공유 (`?share=`) — 내 리스트 탭에서 일원화
- **방문 기록**: 가본 곳 체크, "안 가본 곳만" 필터, 방문 별점·메모
- **랜덤 뽑기** (🎲): 현재 필터 기준으로 랜덤 식당 선택
- **오늘의 발견**: 발견 피드 상단 히어로 카드 (매일 바뀌는 고평점 픽)
...
│   ├── src/
│   │   ├── App.jsx          # 메인 (상태·필터·탭 라우팅 오케스트레이터)
│   │   ├── components/      # BottomNav·RestaurantRail·MyListSpace·RestaurantModal·RestaurantCard·FeaturedCard·icons·Skeleton
│   │   ├── utils/           # format(이모지·키·정렬)·geo(haversine)·collections(발견 레일 규칙)
