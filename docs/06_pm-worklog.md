...
## 6.11 크롤러 결과 출렁임 근본 진단 (커버리지 확대는 보류)

6.10 sweep 수정 후, "왜 회차마다 결과 수가 달라지나"를 근본 진단.

### 진단 결과
- **CI 로그 실측**: 1회 크롤에서 50개 (구×카테고리) 쌍이 **전부 정확히 15개씩** 수집 (min=median=max=15, 총 750/회). 크롤 실패·타임아웃 아님.
- **원인**: `map.kakao.com/?q=` 웹 검색이 **쿼리당 15개로 하드캡**. DOM 조사 결과 페이지 버튼(no2~5)은 존재하나 항상 HIDDEN/disabled — "강남 맛집", "강남구 고기" 같은 밀집 쿼리도 next 비활성. **페이지네이션 자체가 제공 안 됨.**
- **출렁임 메커니즘**: 쿼리당 15개 고정 + 카카오 랭킹이 회차마다 셔플 → 매번 다른 15개 → upsert union으로 DB는 ~1,150까지 누적되나 1회 크롤은 750. 여기에 옛 sweep 카테시안 버그가 "이번에 없는 나머지"를 전부 숨겨 1,110→793 급감 발생.
- **결론**: 출렁임의 실질 범인은 sweep 버그(6.10에서 수정). 건강도 가드로 DB는 이제 ~1,150 안정 유지.

### 커버리지 확대 옵션 (보류 — 필요 시 착수)
쿼리당 15개 캡이라 커버리지를 늘리려면 **쿼리 수를 늘려** 각기 다른 15개를 union하는 수밖에 없음(페이지네이션 불가):
- A. 키워드 변형: "{구} 노포 맛집/술집/백반" 등
